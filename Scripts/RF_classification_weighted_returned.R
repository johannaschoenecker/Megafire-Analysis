...
# vpdmax_1post_abs,tmin_1post_abs,years_since_fire
...
sampled_data <- data %>% sample_frac(0.7)
...
vars_to_keep <- c("dist_unburned","elevation","slope","aspect","tpi","ppt_1post",
...
# 
# saveRDS(rf_model_class, file = "C:/Users/jscho/Documents/Megafires-1985-2023/rf_model_class_returned.rds")

rf_model_class <- readRDS(file = "C:/Users/jscho/Documents/Megafires-1985-2023/rf_model_class_returned.rds")
...
print("pdp interval done")
pdp_tri <- partial(rf_model_class, pred.var = "tri", train = train_data, which.class = "1", prob = TRUE)
print("pdp tri done")
...
pdp_dist_unburned <- partial(rf_model_class, pred.var = "dist_unburned", train = train_data, which.class = "1", prob = TRUE)
print("pdp dist_unburned")
pdp_vpdmin_1post <- partial(rf_model_class, pred.var = "vpdmin_1post", train = train_data, which.class = "1", prob = TRUE)
print("pdp vpdmin_1post")
...
pdp_df_tri$variable <- "tri"
colnames(pdp_df_tri)[2] <- "value"
colnames(pdp_df_tri)[1] <- "indep"
...
pdp_df_vpdmin_1post$variable <- "vpdmin_1post"
colnames(pdp_df_vpdmin_1post)[2] <- "value"
colnames(pdp_df_vpdmin_1post)[1] <- "indep"



pdp_df_master_new <- rbind(pdp_df_elevation,pdp_df_aspect, pdp_df_tri,pdp_df_tpi,pdp_df_dist_unburned, pdp_df_vpdmin_1post)
...
pdp_years_new <- ggplot(pdp_df_master_new, aes(x = indep, y = value)) +
  ...
  facet_wrap(~ variable,nrow = 2, ncol = 3, scales = 'free')  # Facet by the 'variable' column

pdp_df_master_backup <- pdp_df_master_new

# Reorder and rename the 'variable' column
pdp_df_master_new$variable <- recode(pdp_df_master_new$variable,
                                     'dist_unburned' = 'distance to seedsource',
                                     'elevation' = 'elevation',
                                     'aspect' = 'aspect',
                                     'tpi' = 'TPI',
                                     'tri' = 'TRI',
                                     'vpdmin_1post' = 'post-FY min vpd anom'
)
...
## Create partial dependence plots for top 8 variables

# Reduce grid resolution for speed
grid_res <- 10

# Create an empty list for PDP plots
pdp_plots <- list()

# Loop through the top 8 variables (sequential computation)
for (var in top_vars) {
  cat("Computing PDP for:", var, "\n")  # Track progress

  pd <- tryCatch({
    pdp::partial(
      object = rf_model_class,  # Use the model directly
      pred.var = var,
      train = train_data,
      prob = TRUE,
      grid.resolution = grid_res,  # Reduce number of grid points
      which.class = "0"  # Explicitly specify the class
    )
  }, error = function(e) {
    cat("Error in PDP for:", var, ":", conditionMessage(e), "\n")
    return(NULL)  # Skip this variable if an error occurs
  })

  if (!is.null(pd)) {
    pdp_plots[[var]] <- ggplot(pd, aes_string(x = var, y = "yhat")) +
      geom_line(size = 1, color = "darkgrey") +
      labs(title = paste("Partial Dependence of", var),
           x = var,
...
cat("PDP Computation Completed!\n")

saveRDS(pdp_plots, "C:/Users/jscho/Documents/Megafires-1985-2023/pdp_plots_newest.rds")

pdp_plots <- readRDS("C:/Users/jscho/Documents/Megafires-1985-2023/pdp_plots.rds")

custom_theme <- theme_pubr() +labs_pubr()+
  theme(plot.title = element_blank())

# Apply theme to all PDPs
pdp_plots <- lapply(pdp_plots, function(p) p + custom_theme)

# Apply new x-axis labels to each plot
new_x_labels <- c("Elevation","Slope","Aspect","Distance from seed source", "TPI", "PPT anomaly post-fire", "VPDmin anomaly post-fire","Tdmean anomaly post-fire")

pdp_plots <- lapply(seq_along(pdp_plots), function(i) {
  pdp_plots[[i]] + labs(x = new_x_labels[i])  # Update x-axis label
})

pdp_plots <- lapply(pdp_plots, function(p) {
  p + theme(axis.title.y = element_blank())  # Removes only the y-axis label, keeps tick marks & values
})


# Create a single label for the entire grid
y_axis_label <- ggdraw() +
  draw_label("Predicted probability for 20-year conifer recovery failure",
             angle = 90,  # Vertical text
             fontface = "bold", size = 14)

# Arrange the plots in a grid with the shared y-axis label
final_plot <- plot_grid(y_axis_label,  # Shared y-axis label
                        plot_grid(plotlist = pdp_plots, ncol = 4, align = "hv"),
                        rel_widths = c(0.1, 1))  # Adjust spacing

# Arrange the plots in a grid with the shared y-axis label
final_plot <- plot_grid(y_axis_label,  # Shared y-axis label
                        plot_grid(plotlist = pdp_plots[1:6], ncol = 3, align = "hv"),
                        rel_widths = c(0.1, 1))  # Adjust spacing
