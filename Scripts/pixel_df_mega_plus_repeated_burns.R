...
# 🔘 Annual (1985-2023) raster images of all fires in the study region
# 🔘 Raster image of FACTS tree plantings with year as pixel value
# 🔘 Raster image of areas that are managed by the US Forest Service
...
############ Add year of soonest reburn to each pixel and save updated dataframes ###############
...
# Define directories
input_dir <- "C:/Users/jscho/Documents/Megafires-1985-2023-Data/raster_df_mega/"
output_dir <- "C:/Users/jscho/Documents/Megafires-1985-2023-Data/raster_df_mega_repeated/"
raster_dir <- "C:/Users/jscho/Documents/Megafires-1985-2023-Data/Rasters/SN_all_fires_yearly/"

dir_create(output_dir)

# Identify raster files and their years 
raster_files <- dir_ls(raster_dir, regexp = "\\.tif$")
raster_years <- sort(as.integer(gsub("\\D", "", path_file(raster_files))))
names(raster_files) <- raster_years

# Load all rasters and compute union extent 
raster_objs <- lapply(raster_files, rast)

# Compute union of all extents
combined_extent <- Reduce(terra::union, lapply(raster_objs, ext))

common_res <- res(raster_objs[[1]])
common_crs <- crs(raster_objs[[1]])

# Create a blank reference raster with union extent
ref_rast <- rast(extent = combined_extent, resolution = common_res, crs = common_crs)
values(ref_rast) <- NA

# Resample all rasters to the reference raster 
message("Resampling all rasters to unified extent and resolution...")
rasters_resampled <- mapply(function(year, r) {
  r_resampled <- resample(r, ref_rast, method = "near")
  names(r_resampled) <- paste0("fire_", year)
  return(r_resampled)
}, raster_years, raster_objs, SIMPLIFY = FALSE)

# Create raster stack once
raster_stack <- rast(rasters_resampled)
...
for (csv_file in csv_files) {
  ...
  if (!all(c("x", "y") %in% names(df))) {
    warning("Missing x/y columns in: ", csv_file)
    next
  }
  
  # Create points from x/y
  points <- vect(df[, .(x, y)], geom = c("x", "y"), crs = common_crs)
  
  # Extract values from all rasters at once
  extracted <- extract(raster_stack, points)[, -1]  # Drop ID column
  df <- cbind(df, extracted)
  
  # Write output
  out_path <- file.path(output_dir, path_file(csv_file))
  fwrite(df, out_path)
...
### Add binary 'reburn' column to dataframes; 1== yes, there was a reburn after megafire, 0== no, not reburned ###

### Add also if something reburned within 20 years after fire ###
library(data.table)
library(fs)

# Set paths
input_dir <- "C:/Users/jscho/Documents/Megafires-1985-2023-Data/raster_df_mega_repeated/"
output_dir <- "C:/Users/jscho/Documents/Megafires-1985-2023-Data/raster_df_mega_repeated_bin/"
...
for (csv_file in csv_files) {
  ...
  # Check fire_year
  if (!("fire_year" %in% names(df))) {
    warning("Missing fire_year column in: ", csv_file)
    next
  }
  
  fire_year_unique <- unique(df$fire_year)
  if (length(fire_year_unique) != 1 || !is.numeric(fire_year_unique)) {
    warning("Invalid or multiple fire_years in: ", csv_file)
    next
  }
  
  # Find all fire year columns like fire_1985, fire_1986, ...
  year_cols <- grep("^fire_\\d{4}$", names(df), value = TRUE)
  if (length(year_cols) == 0) {
    warning("No 'fire_' year columns found in: ", csv_file)
    next
  }
  
  # Extract year numbers from fire_* columns
  year_numbers <- as.integer(sub("fire_", "", year_cols))
  
  # Create matrix of the fire_* values
  fire_matrix <- as.matrix(df[, ..year_cols])
  
  # Create reburn column: any value > fire_year
  df[, reburn := as.integer(
    apply(fire_matrix, 1, function(x) any(x > fire_year_unique, na.rm = TRUE))
  )]
  
  # Create reburn_20yrs column: any value > fire_year and <= fire_year + 20
  df[, reburn_20yrs := as.integer(
    apply(fire_matrix, 1, function(x) any(x > fire_year_unique & x <= (fire_year_unique + 20), na.rm = TRUE))
  )]
  
  # Save to output
  output_path <- file.path(output_dir, path_file(csv_file))
...
### Add information on tree planting to each dataframe ###
...
# Set directories
input_dir <- "C:/Users/jscho/Documents/Megafires-1985-2023-Data/raster_df_mega_repeated_bin/"
output_dir <- "C:/Users/jscho/Documents/Megafires-1985-2023-Data/raster_df_mega_repeated_planting/"

# Create output directory if it doesn't exist
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

# Raster paths
planting_raster <- "C:/Users/jscho/Documents/Megafires-1985-2023-Data/Rasters/tree_planting_year.tif"
nf_raster <- "C:/Users/jscho/Documents/Megafires-1985-2023-Data/Rasters/megafires_NF.tif"

# Load rasters
r_plant <- rast(planting_raster)
r_nf <- rast(nf_raster)
...
for (csv_file in csv_files) {
  ...
  # Convert to points
  points <- vect(df[, .(x, y)], geom = c("x", "y"), crs = crs(r_plant))
  
  # Extract planting year
  df[, tree_planting_yr := extract(r_plant, points)[,2]]
  
  # Assign tree_planting = 1 only if tree_planting_yr is not NA and >= fire_year
  df[, tree_planting := fifelse(
    is.na(tree_planting_yr) | tree_planting_yr < fire_year, 
    0, 
    1
  )]
  
  # Extract NF OBJECTID and binary flag
  df[, NF_OBJECTID := extract(r_nf, points)[,2]]
  df[, NFS_bin := as.integer(!is.na(NF_OBJECTID))]
  
  # Save output
  fwrite(df, file.path(output_dir, path_file(csv_file)))
}
