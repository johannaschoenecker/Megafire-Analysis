...
plot_23_1 <- ggplot() + 
  # 🔹 Bar plot: Area burned per year
  geom_bar(data = area_burned_long, 
           aes(x = YEAR_, y = area_ha, fill = type), 
           stat = 'identity') +  # ✅ Fill mapped correctly
  
  # 🔹 Points: Number of megafires per year (rescaled to match primary axis)
  geom_point(data = mega_counts_area, 
             aes(x = year, y = count_mega * 100000),  # ✅ Rescale for better visibility
             size = 3, color = "black", alpha=1.0) + 

  # 🔹 Poisson regression smooth line
  geom_smooth(data = mega_counts_area, 
              aes(x = year, y = count_mega * 100000),  # ✅ Rescale to align with primary y-axis
  ...
  # 🔹 Y-axis scaling (Primary: Area burned, Secondary: Number of megafires)
  scale_y_continuous(
  ...
  # 🔹 Custom colors for fire size
  scale_fill_manual(values = c('small_area' = '#E17327', 'mega_area' = '#932191')) + 

  # 🔹 Theme customization (Removing Gridlines)
  theme_bw() +
  theme(
    panel.grid.major = element_blank(),  # ✅ Removes major gridlines
    panel.grid.minor = element_blank(),  # ✅ Removes minor gridlines
...
patch_long <- patch_metrics_mega %>%
  pivot_longer(cols = c(core_area, outer_area_ha),
               names_to = "area_type",
...
)+ annotate("text", x = Inf, y = max(patch_totals$cumulative_area, na.rm = TRUE),
           label = "cumulative total area", color = "darkred", hjust = 1.1, vjust = -0.5, size = 3.5)+ annotate("text", x = Inf, y = max(patch_core_cumulative$cumulative_core_area, na.rm = TRUE),
           label = "cumulative core area", color = "darkorange", hjust = 1.1, vjust = -0.5, size = 3.5) + annotate("text", x = Inf, y = max(patch_outer_cumulative$cumulative_outer_area, na.rm = TRUE),
           label = "cumulative outer area", color = "grey40", hjust = 1.1, vjust = -0.5, size = 3.5)
...
# Define path to your raster directory
raster_dir <- "C:/Users/jscho/Documents/Megafires-1985-2023-Data/Rasters/RdNBR_classified"
...
calculate_lpi <- function(file_path) {
  ...
  # Convert to 'RasterLayer' (landscapemetrics works best with this class)
  rast_r <- raster::raster(rast_obj)

  # Calculate LPI for each class
  lpi_result <- landscapemetrics::lsm_c_lpi(rast_r)

  # Add raster name (without path) to identify
  lpi_result$raster_name <- basename(file_path)
  
  return(lpi_result)
}

# Apply function to all raster files
lpi_list <- lapply(raster_files, calculate_lpi)

# Combine results into a single dataframe
lpi_df <- bind_rows(lpi_list)



# Turn this into a wide format table to add to supplemental information

# Step 1: Extract fire ID from raster_name and join with fire names
lpi_df <- lpi_df %>%
  mutate(
    OBJECTID = as.numeric(str_remove(raster_name, "\\.tif$"))
  ) %>%
  left_join(
    fire_metrics_mega %>% select(OBJECTID, FIRE_NAME),
    by = "OBJECTID"
  )
...
# Step 3: Summarize to one row per fire with total value per class
lpi_summary <- lpi_df %>%
  group_by(`FIRE_NAME`, class_label) %>%
...
lpi_summary <- lpi_summary %>%
  select(`Fire name`, Unchanged, Low, Moderate, High)

lpi_summary <- lpi_summary %>%
  left_join(fire_metrics_mega %>% select(FIRE_NAME, YEAR_), by = c("Fire name" = "FIRE_NAME")) %>%
  rename(`Fire year` = YEAR_) %>%
  select(`Fire name`, `Fire year`, Unchanged, Low, Moderate, High)



fwrite(lpi_summary, "C:/Users/jscho/Documents/Megafires-1985-2023-Data/LPI_all_megafires.csv")

```


# Fig. 3: Patch characteristics
...
patch_data <- patch_metrics_mega %>%
  mutate(n_bins = cut(n,
                      ...
                      labels = c("small", "medium", "large", "very large"),
                      include.lowest = TRUE)) %>%
  filter(n > 2)  # Remove very small patches

# Optional filter for conifer vegetation
# patch_data <- patch_data %>% filter(mode_veg == 7)
...
core_area_summary <- patch_data %>%
  ...
  summarise(core_area_sum = sum(core_area), .groups = "drop")

# Contiguity mean
contiguity_summary <- patch_data %>%
  group_by(OBJECTID, n_bins, rdnbr_class) %>%
  summarise(contiguity_mean = mean(contiguity), .groups = "drop")

# Perimeter-area ratio mean
paratio_summary <- patch_data %>%
  group_by(OBJECTID, n_bins, rdnbr_class) %>%
  summarise(perimeter_area_mean = mean(perimeter_area), .groups = "drop")
...
recode_rdnbr <- function(df) {
  df %>%
    ...
    mutate(
      rdnbr_class = dplyr::recode(as.character(rdnbr_class),
                                  "1" = "low",
                                  "2" = "moderate",
                                  "3" = "high"),
...
plot_core_area <- ggplot(core_area_burned, aes(x = n_bins, y = core_area_sum, fill = rdnbr_class)) +
  geom_boxplot(alpha = 0.5) +
  scale_fill_manual(values = rdnbr_colors_named) +
  labs(x = "Patch size class", y = "Core area within patch (ha)") +
  coord_cartesian(ylim = c(0, 7500)) +
  scale_y_continuous(labels = scales::comma) +
  box_theme


plot_paratio <- ggplot(paratio_burned, aes(x = n_bins, y = perimeter_area_mean, fill = rdnbr_class)) +
  geom_boxplot(alpha = 0.5) +
  scale_fill_manual(values = rdnbr_colors_named) +
  labs(x = "Patch size class", y = "Perimeter:area") +
  box_theme

plot_contiguity <- ggplot(contiguity_burned, aes(x = n_bins, y = contiguity_mean, fill = rdnbr_class)) +
  geom_boxplot(alpha = 0.5) +
  scale_fill_manual(values = rdnbr_colors_named) +
  labs(x = "Patch size class", y = "Contiguity") +
  box_theme
...
patch_data <- patch_data %>%
  mutate(rdnbr_class = factor(rdnbr_class))
...
lineplot_core_area <- ggplot(summary_by_year, aes(x = fire_year, y = mean_core_area, color = rdnbr_class)) +
  geom_point(size = 2.5, alpha = 0.7) +
  geom_smooth(method = "gam", se = TRUE, alpha = 0.2) +
  scale_color_manual(values = rdnbr_colors) +
  labs(y = "Mean core area within patch (ha)", x = "Fire year") +
  line_theme

lineplot_paratio <- ggplot(summary_by_year, aes(x = fire_year, y = mean_perimeter_area, color = rdnbr_class)) +
  geom_point(size = 2.5, alpha = 0.7) +
  geom_smooth(method = "gam", se = TRUE, alpha = 0.2) +
  scale_color_manual(values = rdnbr_colors) +
  labs(y = "Perimeter:area", x = "Fire year") +
  line_theme


lineplot_contiguity <- ggplot(summary_by_year, aes(x = fire_year, y = mean_contiguity, color = rdnbr_class)) +
  geom_point(size = 2.5, alpha = 0.7) +
  geom_smooth(method = "gam", se = TRUE, alpha = 0.2) +
  scale_color_manual(values = rdnbr_colors) +
  labs(y = "Contiguity", x = "Fire year") +
  line_theme


### Combine plots
# Top row (boxplots)
top_combined <- plot_grid(
  plot_paratio + theme(axis.title.x = element_blank()),
  plot_contiguity + theme(axis.title.x = element_blank()),
  plot_core_area + theme(axis.title.x = element_blank()),
  labels = c("A", "B", "C"),
  label_size = 20,
  ncol = 3
)
...
bottom_combined <- plot_grid(
  lineplot_paratio + theme(axis.title.x = element_blank()),
  lineplot_contiguity + theme(axis.title.x = element_blank()),
  lineplot_core_area + theme(axis.title.x = element_blank()),
  labels = c("A", "B", "C"),
  ncol = 3
)

# Add shared x-axis labels
top_labeled <- plot_grid(
  top_combined,
  grid::textGrob("Patch size class", gp = gpar(fontsize = 20)),
  ncol = 1, rel_heights = c(1, 0.05)
)

bottom_labeled <- plot_grid(
  bottom_combined,
  grid::textGrob("Fire year", gp = gpar(fontface = "bold", fontsize = 12)),
  ncol = 1, rel_heights = c(1, 0.05)
)

# Final stack
final_patch_plots <- plot_grid(top_labeled, bottom_labeled, ncol = 1)
...
# Check assumptions
# Normality check for core area
shapiro.test(residuals(aov_core))
qqnorm(residuals(aov_core)); qqline(residuals(aov_core))

leveneTest(core_area_sum ~ interaction(rdnbr_class, n_bins), data = core_area_burned)

library(broom)

# Tidy results
core_tidy <- tidy(aov_core)
paratio_tidy <- tidy(aov_paratio)
contiguity_tidy <- tidy(aov_contiguity)

# Add metric name to each
core_tidy$metric <- "Core area"
paratio_tidy$metric <- "Perimeter-area ratio"
contiguity_tidy$metric <- "Contiguity"

# Combine all into one summary table
anova_summary <- bind_rows(core_tidy, paratio_tidy, contiguity_tidy)

# Optional: View or export
print(anova_summary)


ggsave("C:/Users/jscho/Documents/Megafires-1985-2023/Figures/patches_boxes_new.png", plot = top_labeled, width = 10, height = 7.5, units = "in", dpi = 300)
...
patch_metrics_summary$mean_core_area <- format(patch_metrics_summary$mean_core_area, scientific = FALSE)
patch_metrics_summary$sd_core_area <- format(patch_metrics_summary$sd_core_area, scientific = FALSE)

patch_metrics_mega_vlarge <- patch_metrics_mega %>%
  filter(n_bins == 'very large')

patch_metrics_mega_vlarge_sev <- patch_metrics_mega_vlarge %>%
  filter(rdnbr_class == 3)
...
pixel_df <- fread("C:/Users/jscho/Documents/Megafires-1985-2023-Data/raster_df_mega_repeated_planting/3567.csv", header=TRUE)  
...
pixel_filename <- paste0("C:/Users/jscho/Documents/Megafires-1985-2023-Data/raster_df_mega_repeated_planting/",num,".csv")
